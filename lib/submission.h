...
/*
 * Get the first submission and advance the queue to the next submission. The
 * submission must be handled before pushing to the queue.
 *
 * XXX Move to submission pointers to avoid this fragile interface.
 */
int submission_queue_pop(struct submission_queue *sq, struct submission **out);

/*
 * Get the last submission and advance to the quueue to the next submission.
 * The submission must be initialized before popping from the queue.
 *
 * XXX Move to submission pointers to avoid this fragile interface.
 */
